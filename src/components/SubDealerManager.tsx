...
import { Card, Table, Button, Form, Input, InputNumber, Select, Tag, message, Space, Popconfirm, Row, Col, Spin } from 'antd';
import { Briefcase, Plus, Trash2, Edit3, User, Mail, Phone, MapPin, DollarSign, Zap, Network, Info } from 'lucide-react';
...
interface SubDealerManagerProps {
  currentLevelId: string;
  currentRole: string;
  onViewProfile?: (resellerId: string) => void;
}

export default function SubDealerManager({ currentLevelId, currentRole, onViewProfile }: SubDealerManagerProps) {
  const [subDealers, setSubDealers] = useState<ResellerNode[]>([]);
  const [dealers, setDealers] = useState<ResellerNode[]>([]);
  ...
  // Load and segment all resellers
  const fetchResellerData = async () => {
    ...
    try {
      ...
      // Filter list by role levels
      let filteredSubDealers = data.filter(node => node.role === UserRole.SUB_DEALER);
      let filteredDealers = data.filter(node => node.role === UserRole.DEALER);
      
      // Enforce visibility constraints
      if (currentRole === UserRole.DEALER) {
        filteredSubDealers = filteredSubDealers.filter(node => node.parentResellerId === currentLevelId);
      } else if (currentRole === UserRole.FRANCHISE) {
        const myDealers = filteredDealers.filter(node => node.parentResellerId === currentLevelId).map(d => d.id);
        filteredSubDealers = filteredSubDealers.filter(node => 
          node.parentResellerId && myDealers.includes(node.parentResellerId)
        );
        filteredDealers = filteredDealers.filter(node => node.parentResellerId === currentLevelId);
      }
      
      setSubDealers(filteredSubDealers);
  ...
  const handleSave = async (values: any) => {
    ...
    const payload = {
      ...
      parentResellerId: values.parentResellerId || undefined,
    ...
    try {
      ...
      message.success(`Sub-Dealer "${values.name}" successfully ${editingNode ? 'updated' : 'registered'}.`);
      resetForm();
      fetchResellerData();
    } catch (err: any) {
      message.error(err.message || 'Error saving sub-dealer details.');
    }
  };
  ...
  const handleDelete = async (id: string, name: string) => {
    try {
      ...
      message.success(`Sub-Dealer "${name}" removed from server directory.`);
      fetchResellerData();
    } catch (err: any) {
  ...
  const startEdit = (node: ResellerNode) => {
    ...
    form.setFieldsValue({
      ...
      parentResellerId: node.parentResellerId,
  ...
  const isEligibleToEdit = currentRole === UserRole.ADMIN || currentRole === UserRole.FRANCHISE || currentRole === UserRole.DEALER;

  const columns = [
    {
      title: 'Sub-Dealer Profile',
      dataIndex: 'name',
      key: 'name',
      render: (text: string, record: ResellerNode) => (
        <div className="flex items-center gap-3">
          <div className="bg-[#00b991]/10 border border-[#00b991]/20 p-2 rounded-lg text-[#00b991]">
    ...
    {
      title: 'Parent Dealer Link',
      dataIndex: 'parentResellerId',
      key: 'parentReseller',
      render: (parentId: string) => {
        ...
        return (
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-amber-300 font-medium font-sans">
              {parent ? parent.name : 'Direct Dealer Link'}
            </span>
          </div>
        );
      },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-left">
        <div>
          <h1 className="text-xl font-bold font-display text-slate-100 flex items-center gap-2">
            <Briefcase className="w-5 h-5 text-[#00b991]" />
            <span>Sub-Dealers Network Registry</span>
          </h1>
          <p className="text-[11px] text-slate-400 font-mono">Micro-manage neighbourhood distribution nodes, balance pipelines, parent dealers mapping and active status.</p>
        </div>
        {isEligibleToEdit && !showAddForm && (
          <Button 
          ...
          >
            <PlusOutlined /> Register Sub-Dealer
          </Button>
        )}
      </div>

      {/* Dynamic Creation Form styled to look exactly like Nexus isp CRM */}
      {showAddForm && isEligibleToEdit && (
        <Card 
          ...
          title={
            <div className="flex items-center gap-2 text-slate-100 font-mono text-xs tracking-wide">
              <Network className="w-4 h-4 text-[#00b991]" />
              <span>{editingNode ? `EDITING SUB-DEALER: ${editingNode.name.toUpperCase()}` : 'ADD NEIGHBOURHOOD SUB-DEALER'}</span>
            </div>
          }
          extra={
          ...
          >
            <Row gutter={[20, 20]} className="text-left">
              ...
              <Col xs={24} md={12}>
                <Form.Item 
                  name="name" 
                  label={<span className="text-[12px] font-bold text-slate-300 font-mono">SUB-DEALER BUSINESS NAME <span className="text-rose-500">*</span></span>}
                  rules={[{ required: true, message: 'Sub-dealer name is required' }]}
                >
                  <Input prefix={<Briefcase className="w-4 h-4 text-slate-500" />} placeholder="Ex: Central Sector Sub-Dealer" className="bg-slate-950 border-slate-800 text-slate-100 placeholder-slate-600 rounded-lg h-10" />
                </Form.Item>
              </Col>
              ...
              <Col xs={24} md={12}>
                ...
                >
                  <Input prefix={<User className="w-4 h-4 text-slate-500" />} placeholder="Ex: Haris Ahmed" className="bg-slate-950 border-slate-800 text-slate-100 placeholder-slate-600 rounded-lg h-10" />
                </Form.Item>
              </Col>
              ...
              <Col xs={24} md={12}>
                <Form.Item 
                  name="parentResellerId" 
                  label={<span className="text-[12px] font-bold text-slate-300 font-mono">PARENT DEALER NETWORK ASSOCIATED <span className="text-rose-500">*</span></span>}
                  rules={[{ required: true, message: 'Please connect a parent distributor dealer' }]}
                >
                  <Select 
                    placeholder="Choose parent dealer hub..."
                    popupClassName="dark-dropdown"
                    className="bg-slate-950 border-slate-800 text-slate-100 h-10 select-height-10 rounded-lg"
                  >
                    {dealers.map(d => (
              ...
              <Col xs={24} md={12}>
                ...
                >
                  <Input prefix={<Mail className="w-4 h-4 text-slate-500" />} placeholder="Ex: microhub@subdealer.net" className="bg-slate-950 border-slate-800 text-slate-100 placeholder-slate-600 rounded-lg h-10" />
                </Form.Item>
              </Col>
              ...
              <Col xs={24} md={12}>
                ...
                >
                  <Input prefix={<Phone className="w-4 h-4 text-slate-500" />} placeholder="Ex: +92 345 1112223" className="bg-slate-950 border-slate-800 text-slate-100 placeholder-slate-600 rounded-lg h-10" />
                </Form.Item>
              </Col>
              ...
              <Col xs={24} md={12}>
                <Form.Item 
                  name="location" 
                  label={<span className="text-[12px] font-bold text-slate-300 font-mono">NEIGHBOURHOOD COVERAGE STREETS <span className="text-rose-500">*</span></span>}
                  rules={[{ required: true, message: 'Area list is required' }]}
                >
                  <Input prefix={<MapPin className="w-4 h-4 text-slate-500" />} placeholder="Ex: Block G Street 3-9" className="bg-slate-950 border-slate-800 text-slate-100 placeholder-slate-600 rounded-lg h-10" />
                </Form.Item>
              </Col>
              ...
              <Col xs={24} md={6}>
                <Form.Item 
                  name="balance" 
                  label={<span className="text-[12px] font-bold text-slate-300 font-mono">STARTING WALLET VALUE (PKR)</span>}
                >
              ...
              <Col xs={24} md={24}>
                ...
                >
                  <Select 
                    mode="multiple" 
                    placeholder="Click to select broadband package profiles to allot to this sub-dealer..."
            ...
            <div className="my-4 bg-indigo-950/40 border border-indigo-700/35 p-4 rounded-xl flex items-start gap-2.5 text-left">
              <Info className="text-indigo-400 mt-0.5 shrink-0 w-4 h-4" />
              <p className="text-[11px] text-indigo-300/90 font-medium leading-relaxed">
                Sub-dealers are situated closest to user residences. They inherit local line bandwidth pools and top-up directly from the allocated dealer.
              </p>
            </div>
      ...
      {/* SubDealers Database Records Table */}
