...
import { Zap, Plus, Trash2, Edit3, User, Mail, Phone, MapPin, DollarSign, Shield, Network, Info } from 'lucide-react';
...
export default function DealerManager({ currentLevelId, currentRole, onViewProfile }: DealerManagerProps) {
  ...
  const fetchResellerData = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Failed to fetch reseller registry');
      const data: ResellerNode[] = await res.json();
      
      // Segment list by level roles
      let filteredDealers = data.filter(node => node.role === UserRole.DEALER);
      const filteredFranchises = data.filter(node => node.role === UserRole.FRANCHISE);
  ...
  const handleSave = async (values: any) => {
    ...
    try {
      ...
      message.success(`Dealer "${values.name}" successfully ${editingNode ? 'updated' : 'registered'}.`);
      resetForm();
      fetchResellerData();
    } catch (err: any) {
      message.error(err.message || 'Error saving dealer details.');
    }
  };
  ...
  const handleDelete = async (id: string, name: string) => {
    try {
      ...
      message.success(`Dealer "${name}" removed from server directory.`);
      fetchResellerData();
    } catch (err: any) {
  ...
  const isAdmin = currentRole === UserRole.ADMIN || currentRole === UserRole.FRANCHISE;
  ...
  const columns = [
    ...
    {
      title: 'Parent Franchise',
      dataIndex: 'parentResellerId',
      key: 'parentReseller',
      render: (parentId: string) => {
        const parent = franchises.find(f => f.id === parentId);
        return (
          <div className="flex items-center gap-1.5">
            <Shield className="w-3.5 h-3.5 text-indigo-400" />
            <span className="text-xs text-indigo-300 font-medium font-sans">
              {parent ? parent.name : 'Direct NOC/Admin'}
            </span>
          </div>
        );
      },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-left">
        <div>
          <h1 className="text-xl font-bold font-display text-slate-100 flex items-center gap-2">
            ...
            <span>Dealers Network Control Room</span>
          </h1>
          <p className="text-[11px] text-slate-400 font-mono">Configure secondary dealer nodes, trace linked parent franchises, specify initial margins & balance allocations.</p>
        </div>
        {isAdmin && !showAddForm && (
          <Button 
          ...
          >
            <PlusOutlined /> Add Dealer Account
          </Button>
        )}
      </div>
        ...
        <Card 
          ...
          title={
            <div className="flex items-center gap-2 text-slate-100 font-mono text-xs tracking-wide">
              ...
              <span>{editingNode ? `EDITING DEALER: ${editingNode.name.toUpperCase()}` : 'ADD NEW DEALER ACCOUNT'}</span>
            </div>
          }
          extra={
          ...
          >
            <Row gutter={[20, 20]} className="text-left">
              ...
              <Col xs={24} md={12}>
                <Form.Item 
                  name="name" 
                  label={<span className="text-[12px] font-bold text-slate-300 font-mono">DEALER/BUSINESS NAME <span className="text-rose-500">*</span></span>}
                  rules={[{ required: true, message: 'Dealer name is required' }]}
                >
                  <Input prefix={<Zap className="w-4 h-4 text-slate-500" />} placeholder="Ex: Saddar Area Dealer" className="bg-slate-950 border-slate-800 text-slate-100 placeholder-slate-600 rounded-lg h-10" />
                </Form.Item>
              </Col>
              ...
              <Col xs={24} md={12}>
                ...
                >
                  <Input prefix={<User className="w-4 h-4 text-slate-500" />} placeholder="Ex: Kamran Malik" className="bg-slate-950 border-slate-800 text-slate-100 placeholder-slate-600 rounded-lg h-10" />
                </Form.Item>
              </Col>

              {/* Parent Franchise Selection */}
              <Col xs={24} md={12}>
                <Form.Item 
                  name="parentResellerId" 
                  label={<span className="text-[12px] font-bold text-slate-300 font-mono">PARENT FRANCHISE LINK <span className="text-rose-500">*</span></span>}
                  rules={[{ required: true, message: 'Please connect a parent franchise' }]}
                >
                  <Select 
                    placeholder="Choose parent franchise provider..."
              ...
              <Col xs={24} md={12}>
                ...
                >
                  <Input prefix={<Mail className="w-4 h-4 text-slate-500" />} placeholder="Ex: dealer@alpha.net" className="bg-slate-950 border-slate-800 text-slate-100 placeholder-slate-600 rounded-lg h-10" />
                </Form.Item>
              </Col>
              ...
              <Col xs={24} md={12}>
                ...
                >
                  <Input prefix={<Phone className="w-4 h-4 text-slate-500" />} placeholder="Ex: +92 321 9876543" className="bg-slate-950 border-slate-800 text-slate-100 placeholder-slate-600 rounded-lg h-10" />
                </Form.Item>
              </Col>
              ...
              <Col xs={24} md={12}>
                <Form.Item 
                  name="location" 
                  label={<span className="text-[12px] font-bold text-slate-300 font-mono">DISTRIBUTION HUB/ZONE <span className="text-rose-500">*</span></span>}
                  rules={[{ required: true, message: 'Distribution zone is required' }]}
                >
                  <Input prefix={<MapPin className="w-4 h-4 text-slate-500" />} placeholder="Ex: Saddar Sector B" className="bg-slate-950 border-slate-800 text-slate-100 placeholder-slate-600 rounded-lg h-10" />
                </Form.Item>
              </Col>
              ...
              <Col xs={24} md={24}>
                ...
                >
                  <Select 
                    mode="multiple" 
                    placeholder="Click to select broadband package profiles to allot to this dealer..."
            ...
            <div className="my-4 bg-indigo-950/40 border border-indigo-700/35 p-4 rounded-xl flex items-start gap-2.5 text-left">
              ...
              <p className="text-[11px] text-indigo-300/90 font-medium leading-relaxed">
                Dealers receive credit margins and wallet replenishment directly from their linked Parent Franchise. They can directly activate client routes or assign sub-dealers to micro-manage line infrastructure.
              </p>
            </div>
      ...
      {/* Dealers Database Records Table */}
